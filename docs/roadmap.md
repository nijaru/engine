...
- Experimental Q8_1 activation packing and standalone Q4_K/Q5_K integer-dot GEMV have focused CUDA layout, arithmetic, error-bound, and rejection tests plus kernel-level A/B wins over float warp at representative shapes. Other weight families, batched integer-dot execution, full-model integration/parity, and matched throughput measurements remain unfinished. Packed loads and integer dots are the next kernel hypothesis; they are not automatically qualified by the baseline results.

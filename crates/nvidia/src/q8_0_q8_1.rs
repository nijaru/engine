...
use super::{
    CudaQuantizedKernelError, MAX_BATCH_MEMBERS, Q8_0_BLOCK_BYTES, Q8_0_BLOCK_ELEMENTS,
    Q8_0_VALUE_TYPE, validate_quantized_geometry,
};
use crate::cuda::CudaQuantizedWeight;

const SOURCE: &str = concat!(
    include_str!("kernels/quantized/common.cu"),
    include_str!("kernels/quantized/q8_0_q8_1.cu"),
);
static PTX: OnceLock<Result<Ptx, String>> = OnceLock::new();

/// Experimental integer-dot `Q8_0` GEMV consuming packed `Q8_1` activations.
///
/// Activations use [`crate::CudaQ8_1Quantizer`]'s nine-word blocks. Unlike
/// the K-quant families, `Q8_0` weights are already signed bytes with one
/// `F16` scale per 32-value block, so the integer dot needs no unpacking;
/// arithmetic differs from multiplying fully dequantized activations because
/// of the packed activation scale. Packing is lossy. This kernel is opt-in
/// and does not change model defaults. Requires a GPU supporting the
/// compiled compute-75 target or newer.
pub struct CudaQ8_0Q8_1Gemv {
    stream: Arc<CudaStream>,
    kernel: CudaFunction,
    batch_kernel: CudaFunction,
}

impl CudaQ8_0Q8_1Gemv {
    ...
    pub fn new(stream: Arc<CudaStream>) -> Result<Self, CudaQuantizedKernelError> {
        ...
        let kernel = module
            .load_function("q8_0_q8_1_gemv")
            .map_err(|error| CudaQuantizedKernelError::Driver(error.to_string()))?;
        let batch_kernel = module
            .load_function("q8_0_q8_1_gemv_batch")
            .map_err(|error| CudaQuantizedKernelError::Driver(error.to_string()))?;
        Ok(Self {
            stream,
            kernel,
    ...
    /// quantizer from finite, half-representable scale/sum inputs. No
    /// allocation or host synchronization occurs during this call.
    ///
    /// # Errors
    /// Rejects foreign contexts, malformed `Q8_0` weights, incorrect packed
    ...
    pub fn execute(
        ...
        // Safety: shared geometry validation bounds every weight load, the
        // packed input covers every block the row loop reads, and each row
        // owns one disjoint output element.
        unsafe {
            self.stream
    ...
    ///
    /// # Errors
    /// Rejects foreign contexts, malformed `Q8_0` weights, member counts
    ...
    pub fn execute_batch(
        ...
        if members > MAX_BATCH_MEMBERS {
            return Err(CudaQuantizedKernelError::InvalidWeight(format!(
                "Q8_0 integer-dot batch variant supports at most {MAX_BATCH_MEMBERS} members"
            )));
        }
        let context = self.stream.context().as_ref();
        if context != weight.encoded_data().context().as_ref()

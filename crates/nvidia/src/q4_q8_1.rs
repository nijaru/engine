...
use super::{
    CudaQuantizedKernelError, MAX_BATCH_MEMBERS, Q4_K_BLOCK_BYTES, Q4_K_BLOCK_ELEMENTS,
    Q4_K_VALUE_TYPE, validate_quantized_geometry,
};
use crate::cuda::CudaQuantizedWeight;

const SOURCE: &str = concat!(
    include_str!("kernels/quantized/common.cu"),
    include_str!("kernels/quantized/q4_k_q8_1.cu"),
);
static PTX: OnceLock<Result<Ptx, String>> = OnceLock::new();

/// Experimental integer-dot `Q4_K` GEMV consuming packed `Q8_1` activations.
///
...
pub struct CudaQ4KQ8_1Gemv {
    stream: Arc<CudaStream>,
    kernel: CudaFunction,
    batch_kernel: CudaFunction,
}

impl CudaQ4KQ8_1Gemv {
    ...
    pub fn new(stream: Arc<CudaStream>) -> Result<Self, CudaQuantizedKernelError> {
        ...
        let kernel = module
            .load_function("q4_k_q8_1_gemv")
            .map_err(|error| CudaQuantizedKernelError::Driver(error.to_string()))?;
        let batch_kernel = module
            .load_function("q4_k_q8_1_gemv_batch")
            .map_err(|error| CudaQuantizedKernelError::Driver(error.to_string()))?;
        Ok(Self {
            stream,
            kernel,
    ...
    ///
    /// # Errors
    /// Rejects foreign contexts, malformed `Q4_K` weights, incorrect packed
    ...
    ///
    /// # Errors
    /// Rejects foreign contexts, malformed `Q4_K` weights, member counts
    ...
    pub fn execute_batch(
        ...
        if members > MAX_BATCH_MEMBERS {
            return Err(CudaQuantizedKernelError::InvalidWeight(format!(
                "Q4_K integer-dot batch variant supports at most {MAX_BATCH_MEMBERS} members"
            )));
        }
        let context = self.stream.context().as_ref();
        if context != weight.encoded_data().context().as_ref()
        ...
        // One warp per weight row, matching the single-row kernel; the
        // batch kernel is a separate function in the same module.
        let config = LaunchConfig {
            grid_dim: (output_size.div_ceil(4), 1, 1),
            block_dim: (128, 1, 1),
            shared_mem_bytes: 0,

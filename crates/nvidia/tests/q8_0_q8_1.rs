    ...
    (0..ROWS * (INPUTS / 32))
        .map(|seed| {
            ...
            let quants = std::array::from_fn(|index| {
                i8::try_from((index * 11 + seed * 5) % 61).unwrap() - 30
            });
            let mut encoded = [0_u8; 34];
            encoded[0..2].copy_from_slice(&d_bits.to_le_bytes());
            for (index, quant) in quants.iter().enumerate() {
                encoded[2 + index] = quant.to_ne_bytes()[0];
            }
            Block {
                encoded,
                quants,
...
fn encode_half(value: f32) -> u16 {
    static POSITIVE_FINITE: std::sync::LazyLock<Vec<f64>> =
        std::sync::LazyLock::new(|| (0_u16..=0x7bff).map(decode_half).collect());
    ...
    blocks
        ...
        .map(|row| {
            ...
            for (block_index, block) in row.iter().enumerate() {
                let words = &packed[block_index * 9..][..9];
                ...
                for index in 0..32 {
                    let weight_q = block.quants[index];
                    let activation_q = q[index];
                    integer_dot += i32::from(weight_q) * i32::from(activation_q);
                    let x = f64::from(input[block_index * 32 + index]);
                    result.float_output += block.d * f64::from(weight_q) * x;
                    result.packing_error_bound += block.d.abs()
                        * f64::from(weight_q.abs())
                        * (scale * f64::from(activation_q) - x).abs();
                    result.arithmetic_magnitude +=
                        (block.d * f64::from(weight_q) * scale * f64::from(activation_q)).abs();
                }
                result.packed_output += block.d * scale * f64::from(integer_dot);
                result.arithmetic_magnitude += (block.d * scale * f64::from(integer_dot)).abs();
            }
            result
        })
        .collect()
...
fn exact_activations_and_fixture_metadata_match_float_arithmetic() {
    ...
    for block in &blocks {
        let decoded = engine_gguf::dequantize_block(8, &block.encoded).unwrap();
        for (index, value) in decoded.iter().enumerate() {
            assert_eq!(f64::from(*value), block.d * f64::from(block.quants[index]));
        }
    }
...
mod gpu {
    ...
    use engine_nvidia::{
        CudaQ8_0Q8_1Gemv, CudaQ8_1Quantizer, CudaQuantizedKernelError, CudaWeightStore,
    };
    use std::io::Cursor;

    #[test]
    #[ignore = "requires CUDA GPU and NVRTC"]
    fn q8_0_dp4a_matches_packed_host_arithmetic_and_float_error_bound() {
        let context = CudaContext::new(0).unwrap();
        let stream = context.default_stream();
        let kernel = CudaQ8_0Q8_1Gemv::new(stream.clone()).unwrap();
        ...
        let spec =
            WeightTensorSpec::new("q8", vec![INPUTS as u64, ROWS as u64], DataType::F32).unwrap();
        let mut store = CudaWeightStore::new(stream.clone());
        store
            .materialize_quantized(spec, 8, encoded.len() as u64, &mut Cursor::new(encoded))
            .unwrap();
        let weight = store.quantized_tensor("q8").unwrap();
    ...
    #[test]
    #[ignore = "requires CUDA GPU and NVRTC"]
    fn q8_0_rejects_invalid_storage() {
        let context = CudaContext::new(0).unwrap();
        let stream = context.default_stream();
        let kernel = CudaQ8_0Q8_1Gemv::new(stream.clone()).unwrap();
        let blocks = fixture();
        ...
        let weight = store.quantized_tensor("q8").unwrap();
        let short = stream
            .clone_htod(&vec![0_u32; INPUTS / 32 * 9 - 1])
            .unwrap();
        ...
        let long_output = stream.clone_htod(&[0.0_f32; ROWS + 1]).unwrap();
        let mut long_output = long_output;
        assert!(matches!(
            kernel.execute(weight, &short, &mut long_output),
            Err(CudaQuantizedKernelError::InputLength { .. })
        ));
    ...
    #[test]
    #[ignore = "requires CUDA GPU and NVRTC"]
    fn q8_0_batch_matches_per_member_host_arithmetic() {
    ...
    #[test]
    #[ignore = "requires CUDA GPU and NVRTC"]
    fn q8_0_batch_rejects_invalid_storage() {
        let context = CudaContext::new(0).unwrap();
        let stream = context.default_stream();
        let kernel = CudaQ8_0Q8_1Gemv::new(stream.clone()).unwrap();
        let spec = WeightTensorSpec::new("q8", vec![32, 1], DataType::F32).unwrap();
        let mut store = CudaWeightStore::new(stream.clone());
        store
            .materialize_quantized(spec, 8, 34, &mut Cursor::new(fixture()[0].encoded))
            .unwrap();
        let weight = store.quantized_tensor("q8").unwrap();
        // One member packs 32 inputs into 9 words with one output row.
        let input = stream.alloc_zeros::<u32>(2 * 9).unwrap();
        let mut output = stream.clone_htod(&[1.0_f32, 2.0]).unwrap();
        assert!(matches!(
        ...
        let short = stream.alloc_zeros::<u32>(2 * 9 - 1).unwrap();
        assert!(matches!(

    ...
    (0..ROWS * (INPUTS / 256))
        .map(|seed| {
            ...
            let quants = std::array::from_fn(|index| {
                u8::try_from((index * 7 + seed * 3 + index / 32) % 32).unwrap()
            });
            for group in 0..8 {
                for index in 0..32 {
                    let quant = quants[group * 32 + index];
                    encoded[48 + (group / 2) * 32 + index] |= (quant & 15) << ((group % 2) * 4);
                    encoded[16 + index] |= ((quant >> 4) & 1) << group;
                }
            }
            Block {
                encoded,
...
fn exact_activations_and_fixture_metadata_match_float_arithmetic() {
    ...
    for block in &blocks {
        let decoded = engine_gguf::dequantize_block(13, &block.encoded).unwrap();
...
mod gpu {
    ...
    use engine_nvidia::{
        CudaQ5KQ8_1Gemv, CudaQ8_1Quantizer, CudaQuantizedKernelError, CudaWeightStore,
    };
    use std::io::Cursor;

    #[test]
    #[ignore = "requires CUDA GPU and NVRTC"]
    fn q5_k_dp4a_matches_packed_host_arithmetic_and_float_error_bound() {
    ...
    #[test]
    #[ignore = "requires CUDA GPU and NVRTC"]
    fn q5_k_dp4a_rejects_invalid_storage() {
        let context = CudaContext::new(0).unwrap();
        let stream = context.default_stream();
        let kernel = CudaQ5KQ8_1Gemv::new(stream.clone()).unwrap();
